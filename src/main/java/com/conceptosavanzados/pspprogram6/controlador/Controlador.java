 ...
 */
package com.conceptosavanzados.pspprogram6.controlador;
...
public class Controlador extends HttpServlet 
{
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
    {
        ...
        String dof = (String) request.getParameter("dof");
        String errorAceptable = (String) request.getParameter("errorAceptable");
        try 
        {
            if (p.isEmpty() || errorAceptable.isEmpty() || dof.isEmpty()) 
            {
                throw new Exception("Los datos ingresados no pueden ser nulos ni vacíos");
            }
            CalcularIntegralError resultado = new CalcularIntegralError();
            resultado.setP(Double.parseDouble(p));
            resultado.setGradosLibertdad(Integer.parseInt(dof));
            resultado.setErrorAceptable(Double.parseDouble(errorAceptable));
            resultado.calcularX();
